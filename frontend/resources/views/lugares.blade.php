...
<div class="row" style="margin-left: 5%; margin-right: 1%; margin-top: 4%;">
    <div class="d-flex flex-wrap col-xl-8" style="overflow-y: scroll; height: 400px;">
        @foreach ($lugares as $lugar)
            <div class="card text-bg-dark" style="width: 260px; height: 250px; padding: 3px;">
                <img src="{{ $lugar['image'] }}" class="card-img"
                    style="width: 100%; height: 75%; filter: brightness(0.5);" alt="...">
                <div class="card-img-overlay">
                    <h2 class="card-title" style="color: white">{{ $lugar['Nombre'] }}</h2>
                </div>
                <div class="row" style="margin-top: 3px;">
                    <div class="col-xl-6">
                        <button type="submit"
                            onclick="cambiaValores('{{ $lugar['Nombre'] }}','{{ $lugar['image'] }}','{{ $lugar['Latitud'] }}','{{ $lugar['Longitud'] }}','{{ $lugar['id_poblacion'] }}',{{ $lugar['id'] }})"
                            style="background-color: rgb(73, 150, 73); color:white; width: 110px; margin-left:1px;"
                            class="btn ">Ver info</button>
                    </div>
                    <form class="col-xl-6" action="/lugares/delete/{{ $lugar['id'] }}" method="POST">
                        {{ csrf_field() }}
                        @method('DELETE')
                        <div>
                            <button type="submit"
    ...
    <div class="col-xl-4">
        <form action="/lugares" method="POST" id="formAdd">
            {{ csrf_field() }}
